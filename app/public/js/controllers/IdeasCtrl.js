angular.module('pebbleidea')
  ...
  function($scope, $rootScope, $Ideas) {
    ...
    $scope.getDisplayItems = function() {
      return $scope.displayIdeas;
    }


    // This variable tells us if we've recieved a new idea or not
    $rootScope.$on('castVote', function(event, data) {
      $scope.currentIndex++;
    });

    $scope.$watch('currentIndex', function(newValue, oldValue) {
      if (newValue !== oldValue) {
        $scope.displayIdeas = $Ideas.getIdeas($scope.currentIndex, $scope.totalIdeasToDisplay);
      }
    });
